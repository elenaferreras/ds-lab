...
- `Root` — circle frame, `radius/full`, `color/border/subtle` background fill, **`clip content: true`** (essential — without this the image overflows the circle). The `Root` frame itself must also have `radius/full` applied so clipping is circular.
  ...
  - `Fallback` — absolute fill, `color/border/subtle` bg, `color/text/secondary` text, initials centered, `radius/full`. Visible when `src = false`; hidden when `src = true`.
  - `Alt` — text node, `visibility: true`, **`opacity: 0`**, font `font/family-base`, size `font/size-sm`, color `color/text/secondary`, positioned inside the frame. Opacity 0 (not `visibility: false`) is mandatory — Figma requires the layer to be in the visible layer tree to accept a component property binding. A hidden layer cannot be bound.
...
**Typography:**
- Input text: `font/size-sm` (14px), `font/family-base`, `color/text/primary`
- Placeholder text: `font/size-sm` (14px), `color/text/disabled` (#bfbfbf)
- Label: `font/size-sm` (14px), `font/weight-medium`, `color/text/primary`
- Helper text: `font/size-xs` (12px), `color/text/secondary` (#8c8c8c)
- Error text: `font/size-xs` (12px), `color/feedback/danger` (#ff4d4f)
...
|---|---|---|---|---|
| `default` | `color/text/primary` (#000) | `color/text/inverse` (#fff) | `color/text/primary` at 80% | none |
| `success` | `color/feedback/success` 12% tint over `color/surface/base` | `color/text/primary` (#000) | `color/feedback/success` 40% tint | `CheckCircleOutlined`, `color/feedback/success` |
| `warning` | `color/feedback/warning` 12% tint over `color/surface/base` | `color/text/primary` (#000) | `color/feedback/warning` 40% tint | `WarningOutlined`, `color/feedback/warning` |
| `danger` | `color/feedback/danger` 10% tint over `color/surface/base` | `color/text/primary` (#000) | `color/feedback/danger` 35% tint | `XCircleOutlined`, `color/feedback/danger` |
...
- All SVG path/stroke/fill layers must have their color bound to the `Semantic` Variable **`color/text/primary`** as the default — consuming components override this at the instance level
...
|---|---|---|
| `Arrow` | Vector (fillRule: evenodd) | fill: `color/text/primary` |
...
| Layer name | Type | Attribute |
|---|---|---|
| `Circle` | Ellipse | stroke: `color/text/primary`, strokeWidth: 1.2, opacity: 0.5 |
| `Check` | Vector | stroke: `color/text/primary`, strokeWidth: 1.2, lineCap: round, lineJoin: round |

#### CloseOutlined

| Layer name | Type | Attribute |
|---|---|---|
| `Cross` | Vector (two diagonal lines as one path) | stroke: `color/text/primary`, strokeWidth: 1.2, lineCap: round |

#### PlusOutlined

| Layer name | Type | Attribute |
|---|---|---|
| `Plus` | Vector (vertical + horizontal lines as one path) | stroke: `color/text/primary`, strokeWidth: 1.2, lineCap: round |

#### WarningOutlined

| Layer name | Type | Attribute |
|---|---|---|
| `Triangle` | Vector | stroke: `color/text/primary`, strokeWidth: 1.2, lineJoin: round, opacity: 0.5 |
| `Stem` | Vector | stroke: `color/text/primary`, strokeWidth: 1.2, lineCap: round |
| `Dot` | Ellipse (cx: 7, cy: 10.3, r: 0.65) | fill: `color/text/primary` |
...
After placing the instance, **override the fill/stroke Variable on the instance's vector layers** to match the color that component uses for that icon. Do not leave the instance using the icon's default `color/text/primary` — override it at the instance level using the Variable listed below.
...
| `Toast` (success) | `CheckCircleOutlined` | icon layer | `color/feedback/success` |
| `Toast` (warning) | `WarningOutlined` | icon layer | `color/feedback/warning` |
| `Toast` (danger) | `XCircleOutlined` | icon layer | `color/feedback/danger` |
